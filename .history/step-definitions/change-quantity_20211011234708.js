...
module.exports = function () {
  ...
  this.Then(/^the quantity should increase after clicking \+ button$/, async function () {
    let countBefore = +((await (await driver.findElement(By.css('input.CartQuantityInputField_quantity-input__2dyMe'))).getAttribute("value")).split(" ")[0]);
    await driver.findElement(by.css('[aria-label="Öka antal"]')).click();
    let countAfter = +((await (await driver.findElement(By.css('input.CartQuantityInputField_quantity-input__2dyMe'))).getAttribute("value")).split(" ")[0]);
